...
public class SlidingWindowMedian {
    ...
    public static void main(String[] args) {
        ...
        double[] doubles = slidingWindowMedian.medianSlidingWindow(nums, k);
    ...
    public double[] medianSlidingWindow(int[] nums, int k) {
        ...
        for (int i = 0; i < nums.length; i++) {
            ...
            // usuń element, który wyszedł poza okno
            if (i >= k) {
                removeOutdated(i - k, lower, higher);
            }

            // dopiero gdy mamy pełne okno
            if (i >= k - 1) {
    ...
    // usuwa element o danym indeksie z kopców
