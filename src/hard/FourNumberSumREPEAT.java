...
public class FourNumberSumREPEAT {
    ...
    // Check on piece of paper 15/12/2021
    ...
    public static List<Integer[]> fourNumberSum(int[] array, int targetSum) {
        ...
        for (int i = 1; i < array.length - 1; i++) {
            for (int j = i + 1; j < array.length; j++) {
                int currentSum = array[i] + array[j]; // 1 + 2 = 3
                int difference = targetSum - currentSum; // 16 - 3 = 13
            ...
            for (int k = 0; k < i; k++) {
                int currentSum = array[i] + array[k]; // 1 + -1 = 0
